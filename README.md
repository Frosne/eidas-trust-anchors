...
To extract qualified signature anchors instead, specify the relevant extension (e.g. `--extension QCForESig` or `--extension ForeSignatures`). When filtering for `QCForESig` (or `QCForESeal`), the parser also accepts CA/QC services that only advertise the older `ForeSignatures` (or `ForeSeals`) URI, since some national lists have not yet migrated to the newer qualifier names.
